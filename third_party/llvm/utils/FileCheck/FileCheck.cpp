...
struct MarkerStyle {
    ...
    {
        assert((!FiltersAsError || !Note.empty()) && "expected error diagnostic to have note");
    }
};
    ...
    unsigned& LabelWidth)
{
    ...
    for (auto DiagItr = Diags.begin(), DiagEnd = Diags.end(); DiagItr != DiagEnd;
        ...
        auto CheckLineAndCol = SM.getLineAndColumn(DiagItr->CheckLoc, CheckBufferID);
        llvm::raw_string_ostream Label(A.Label);
        Label << GetCheckTypeAbbreviation(DiagItr->CheckTy) << ":";
        if (CheckBufferID == CheckFileBufferID)
            Label << CheckLineAndCol.first;
        else if (ImpPatBufferIDRange.first <= CheckBufferID && CheckBufferID < ImpPatBufferIDRange.second)
        ...
        if (!DiagItr->Note.empty()) {
            ...
            // component.
            if (DiagItr->InputStartLine == DiagItr->InputEndLine && DiagItr->InputStartCol == DiagItr->InputEndCol)
                A.Marker.Lead = ' ';
        }
        if (DiagItr->MatchTy == FileCheckDiag::MatchFoundErrorNote) {
            assert(!DiagItr->Note.empty() && "expected custom note for MatchFoundErrorNote");
            A.Marker.Note = "error: " + A.Marker.Note;
        }
        A.FoundAndExpectedMatch = DiagItr->MatchTy == FileCheckDiag::MatchFoundAndExpected;
        ...
        if (DiagItr->InputStartLine == DiagItr->InputEndLine) {
            ...
            A.InputEndCol = std::max(DiagItr->InputStartCol + 1, DiagItr->InputEndCol);
            Annotations.push_back(A);
        } else {
            assert(DiagItr->InputStartLine < DiagItr->InputEndLine && "expected input range not to be inverted");
    ...
    unsigned LabelWidth)
{
    ...
    ColorMode TheColorMode = WithColor(OS).colorsEnabled() ? ColorMode::Enable : ColorMode::Disable;
    ...
    for (unsigned Line = 1;
        ...
        raw_ostream* LineOS;
        if ((!PrevLineInFilter || PrevLineInFilter + DumpInputContext < Line) && (NextLineInFilter == UINT_MAX || Line + DumpInputContext < NextLineInFilter))
...
int main(int argc, char** argv)
{
    ...
    DumpInputValue DumpInput = DumpInputs.empty() ? DumpInputFail : *llvm::max_element(DumpInputs);
    DumpInputFilterValue DumpInputFilter;
    ...
    unsigned DumpInputContext = DumpInputContexts.empty() ? 5 : *llvm::max_element(DumpInputContexts);
    ...
    ErrorOr<std::unique_ptr<MemoryBuffer>> CheckFileOrErr = MemoryBuffer::getFileOrSTDIN(CheckFilename, /*IsText=*/true);
    ...
    unsigned CheckFileBufferID = SM.AddNewSourceBuffer(MemoryBuffer::getMemBuffer(
    ...
    ErrorOr<std::unique_ptr<MemoryBuffer>> InputFileOrErr = MemoryBuffer::getFileOrSTDIN(InputFilename, /*IsText=*/true);
    if (InputFilename == "-")
    ...
    int ExitCode = FC.checkInput(SM, InputFileText,
        ...
        ? EXIT_SUCCESS
        : 1;
    if (DumpInput == DumpInputAlways || (ExitCode == 1 && DumpInput == DumpInputFail)) {
        errs() << "\n"
