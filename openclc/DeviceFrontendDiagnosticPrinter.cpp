//===--- DeviceFrontendDiagnosticPrinter.cpp - Diagnostic Printer -------------------===//
//
...
DeviceFrontendDiagnosticPrinter::DeviceFrontendDiagnosticPrinter(raw_ostream& os,
...
DeviceFrontendDiagnosticPrinter::~DeviceFrontendDiagnosticPrinter()
{
    if (OwnsOutputStream)
        delete &OS;
}

void DeviceFrontendDiagnosticPrinter::BeginSourceFile(const LangOptions& LO,
...
void DeviceFrontendDiagnosticPrinter::EndSourceFile()
{
    TextDiag.reset();
}
    ...
    const DiagnosticOptions& DiagOpts)
{
    ...
    if (DiagOpts.ShowOptionNames) {
        ...
        if (Level == DiagnosticsEngine::Error && DiagnosticIDs::isBuiltinWarningOrExtension(Info.getID()) && !DiagnosticIDs::isDefaultMappingAsError(Info.getID())) {
            OS << " [-Werror";
            Started = true;
        }
    ...
    if (DiagOpts.ShowCategories) {
        unsigned DiagCategory = DiagnosticIDs::getCategoryNumberForDiag(Info.getID());
        if (DiagCategory) {
            OS << (Started ? "," : " [");
            Started = true;
...
void DeviceFrontendDiagnosticPrinter::HandleDiagnostic(DiagnosticsEngine::Level Level,
    const Diagnostic& Info)
{
    ...
    // Check if this diagnostic is about an #include failure for system headers that should be skipped.
    // there is '#include <' present and NOT '.clh'
    if (OutStr.find(".h' file not found") != std::string::npos) {
        return;
    }
